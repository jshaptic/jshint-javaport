...
public class Cli
{
		...
		if (config.test("overrides"))
		{
			if (StringUtils.isNotEmpty(file))
			{
				for (String pattern : config.get("overrides").keys())
				{
					...
					if (Minimatch.match(path.normalize(file), pattern, Minimatch.NO_CASE | Minimatch.MATCH_BASE))
					{
	...
	public UniversalContainer loadConfig(String fp) throws ExitException
	{
		...
		try
		{
			...
			{
				UniversalContainer baseConfig = loadConfig(path.resolve(config.asString("dirname"), config.asString("extends")));
