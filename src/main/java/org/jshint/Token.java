...
public final class Token implements Cloneable {
	...
	private boolean isDelim = false; // JSHINT_BUG: this property only used to write values, not to read can be
										// removed
	...
	public void setTokenName(Token tokenName) {
	...
	// PORT INFO: special setter, which is used to mark, that there is functor
	// object in property 'right'
	...
	public void setFirstTokens(Token... tokens) {
		firstTokens = tokens != null ? new ArrayList<>(Arrays.asList(tokens)) : new ArrayList<>();
	}

	public void setFirstTokens(List<Token> tokens) {
		firstTokens = tokens != null ? new ArrayList<>(tokens) : new ArrayList<>();
	}
	...
	static class Meta {
		...
		Meta(boolean es5, boolean isFutureReservedWord, boolean strictOnly, boolean moduleOnly,
				Function<Token, IntFunction<IntFunction<Token>>> nud) {
			setES5(es5);
	...
	Token led(Token _this, int context, Token t) throws JSHintException {
		return led.apply(_this).apply(context).apply(t);
	}
