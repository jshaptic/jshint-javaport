...
public class CliWrapper extends Cli
{
	...
	private static PrintStream SYSTEM_OUT_STUB = new PrintStream(new OutputStream() { @Override public void write(int b) {} });
	...
	public boolean run(RunOptions opts) throws JSHintException, ExitException, IOException
	{
		...
		if (!runStub) return super.run(opts);
	...
	public int interpret(String... args)
	{
		...
		if (!stdinStub) return super.interpret(args);
	...
	public void stdinSend(String... lines)
	{
		if (stdinStub)
		{
			for (String l : lines) stdinLines += l + "\n";
		}
    }
